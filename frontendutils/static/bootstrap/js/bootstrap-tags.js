...
(function() {
	...
	jQuery(function() {
		$.tags = function(element, options) {
			...
			this.tagClass = (options.tagClass != null ? options.tagClass : 'label-info');
			this.promptText = (options.promptText != null ? options.promptText : 'Press enter after each tag...');
			...
			this.adjustInputPosition = function() {
				...
				pTop = tagPosition != null ? tagPosition.top : "4px";
				$('.tags-input', _this.$element).css({
					paddingLeft: pLeft,
					paddingTop: "2px",
					width: this.$element.outerWidth(false) - pLeft
				});
			...
			this.init = function() {
				this.input = $('input.tags-input', this.$element);
...
$(function() {
	...
	var options = {};
	var suggestions = $taglist.data('suggestions');
	if (suggestions)
		options.suggestions = suggestions;
	tags = $taglist.tags(options);
	$('form').submit(function(){
		$('.tag-list', this).each(function (index) {
			var $this = $(this);
			var id = $this.attr('data-id');
			var tagin = $('.tags-input', $this);
			if (tagin.val())
				tagin.trigger({type:'keydown', which: 13});
			$('#'+$this.data('id')).val(tags.getTags());
		});
	});
});